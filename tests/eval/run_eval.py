...
Mock 编译规则说明:MockCompiler 默认规则模拟"缺引用"型错误(出现 Kingdee.BOS /
AbstractOperationServicePlugIn 字面量即命中),真实生成代码必然含这些字面量,
属误报;评估用规则(EVAL_MOCK_RULES)只拦截真实生成缺陷:未渲染的 {{TOKEN}}
占位符残留 / 空代码。真实编译质量门在 w5(msbuild),此处仅做回归基线。
"""
import json
import tempfile
...
def run_eval(llm, store: ArtifactStore, cases_dir: Path, compile_client, rag=None) -> dict:
    """跑全部 case:w3 生成 → compile_client 编译 → eval 报告。

    报告 = {"total", "compile_pass_rate", "review_reject_rate", "results"}。
    results 每项 {"id", "plugin_type", "compiled", "review_passed", "errors"}。
    ...
    for f in sorted(Path(cases_dir).glob("*.json")):
        ...
        sub = Subtask(id=case_id, plugin_type=case["plugin_type"],
        ...
        results.append({"id": case_id, "plugin_type": case["plugin_type"],
