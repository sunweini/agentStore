...
- **访问入口**:nginx 80 端口 `/admin/` 路径 → 反代 admin 容器根。路由已**永久写进 sentiment 的 `nginx.conf` 源文件**(`agents/sentiment_query_agent/deploy/nginx.conf`),sentiment 部署也自动带上,无覆盖 hack。
...
nginx 路由永久在 sentiment nginx.conf 里,无需单独回滚(admin 容器停后 `/admin/` 反代 502,不影响 sentiment 其他路由)。
