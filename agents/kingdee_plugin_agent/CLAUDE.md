...
- **配 embedding 模型**:`.env` 的 `EMBEDDING_*` 组(见 `.env.example`)—— `EMBEDDING_PROVIDER=huggingface`(默认,本地 bge-small-zh-v1.5,离线可用)或 `openai-compatible`(远程 OpenAI 兼容服务,如团队 openclaw memorySearch `http://10.33.17.234:32320` + `Qwen/Qwen3-Embedding-8B`,`EMBEDDING_BASE_URL` 必填)。**换 embedding 模型后必须 drop `data/kingdee-rag` 重灌(向量空间变更,旧向量全部失效)**:`seed_load` + `ingest --seed-internal` + 官方 URL 重灌,流程见 knowledge-steward 维护手册 §5。
- **接真实金蝶环境**:`.env` 配 `KD_BASE_URL/KD_USERNAME/KD_PASSWORD/KD_DATA_CENTER` 4 项(硬门槛:CLI 缺 KD_BASE_URL exit 1;API 4 项全校验,缺任一 503;`KD_DATA_CENTER` 为**账套 ID** 即 ValidateUser acctID,非账套名,可选 `KD_LCID` 默认 2052)。认证走 **ValidateUser 预登录**(`tools/kingdee_api.py`):先 POST AuthService.ValidateUser 得 `KDSVCSessionId`,后续请求带 Cookie `kdservice-sessionid=`、请求体 form-urlencoded `data=JSON`,会话失效自动重登 1 次;旧式「请求体带 userName/password/dc」在真实实例已失效(实测报「会话信息已丢失」),勿回退;httpx 0.28+ 默认 HTTP/2 金蝶不支持,客户端已显式 http1=True。**联调状态(2026-08-10 真实实例 10.33.17.130 实测)**:ValidateUser 登录 / ExecuteBillQuery / QueryBusinessInfo 三端点 ✅ 可用(`get_form_fields` 真实返回 337 字段,会话失效自动重登 ✅);官方 SDK 支持列表(2024-06 手册)无 GetFormOperations/QueryBusinessObjects,两占位方法已删。编译服务配 `COMPILE_SERVICE_URL`(缺省 http://localhost:8000)。**编译服务部署在 Windows(金蝶服务器本机或同网段 Windows 机器)**:前置装 Python 3.10 + .NET Framework 4.8 Developer Pack;金蝶 BOS DLL 从**金蝶服务器 WebSite\bin** 拷贝到 `compile_service\build\references\`(**商业软件,内部使用需确认授权,勿公开分发**;可用 `fetch_kingdee_dlls.ps1` + `KINGDEE_BIN_DIR` env 拷贝);启动 `start_compile.bat`(环境变量 `COMPILE_SERVICE_REQUIRES_DLLS=1` + `REFS_DIR` 指向 references 目录 + `TARGET_FRAMEWORK=v4.8`;可选 `MSBUILD_PATH` 显式 msbuild —— 缺省自动探测:PATH 的 msbuild(VS 环境)→ 兜底 `C:\Windows\Microsoft.NET\Framework64\v4.0.30319\MSBuild.exe` Framework 自带,无 VS 也能编;`FRAMEWORK_MSBUILD_PATH` 覆盖该兜底路径,系统盘非 C: 时用;`COMPILE_ARTIFACT_DIR` 配 DLL 留存目录),schtasks 计划任务保活;编译服务全部路径**代码相对默认 + 环境变量可覆盖,零硬编码部署路径**;完整步骤见 `docs/kingdee-plugin-agent/windows-deployment.md`。**E2E 门已达成**:bill/service/list 三类型样例在 Windows Server 2016 金蝶服务器(WebSite\bin DLL + .NET 4.8 DevPack + Framework MSBuild)真实编译通过并产出 DLL;API 鉴权配 `KINGDEE_API_KEY`。
