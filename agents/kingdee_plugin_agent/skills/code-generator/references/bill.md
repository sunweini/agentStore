...
- **服务调用不编造(库存查询/跨单据读取等外部 API)**:方法名/参数/返回字段
  必须来自 guide 检索或元数据确认;无来源 → TODO 占位 + return 默认值 +
  注释"签名未在元数据/guide 确认,禁止编造",好例/坏例对比见 SKILL.md
  「禁止编造 API」一节(签名能说出处才写,说不出处 = 没来源 = TODO)。
