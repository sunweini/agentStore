...
DEFAULT_NOTES = [
    ...
    ("频次色标", "红=日级 橙=快讯小时级 黄=周级 灰=双周/月级。critical 且涉人员安全的组建议独立快讯管道，命中即推送，不等轮询"),
    ("双轨三式结构", "全量轨(a)=实体键宽召回；精准轨(b)=实体键 AND 风险词，只抓负面，可高频；"
                  "不点名轨(c)=地名键 AND 行业词 AND 外资标识，抓同区域未点名负面，命中多落在 context 层"),
