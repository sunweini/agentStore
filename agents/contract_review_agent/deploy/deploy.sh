...
# compose 参数:独立项目名 deploy-contract-review-agent(命名规范 deploy-<agent>,
# 避免与同机 sentiment/mcp-gateway 的 "deploy" 项目容器名冲突,见 docker-compose.yml 注释)
# 默认生产 compose;COMPOSE_FILE 追加覆盖。端口由 CONTRACT_PORT 驱动(默认 8000)。
_COMPOSE_ARGS=(-p deploy-contract-review-agent -f agents/contract_review_agent/deploy/docker-compose.yml)
...
echo "== 4/6 build + up =="
"${SSH_CMD[@]}" "cd $REMOTE_DIR && CONTRACT_PORT=$PORT docker compose ${_COMPOSE_ARGS[*]} up -d --build"
