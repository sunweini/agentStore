...
- **部署命名规范**:compose 项目名 `deploy-contract-review-agent`(规范 `deploy-<agent>`,见根 CLAUDE.md 架构约定),端口 `CONTRACT_PORT` 环境变量驱动(生产 8002 / 测试 8001,8000 归 sentiment);deploy 用 `bash agents/contract_review_agent/deploy/deploy.sh`,生产 `PORT=8002`、测试 `PORT=8001`。详见 deploy/README.md。
