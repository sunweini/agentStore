...
- **语义检索**(审核节点),两层:
  1. **领域过滤**(硬过滤):按 contract_type 限定法条集合(如劳动合同→劳动法+劳动合同法域;买卖/租赁合同→合同编域),缩小检索空间、法条多时保证精度。
  2. **向量 + 关键词混合**(软排序):域内 BM25 + embedding 检索(RRF 融合,复用 `common/rag.py` 现成实现)→ top-K 相关法条片段 → 注入审核 prompt。例:合同"违约金 5%"命中《民法典》第五百八十五条。
- **精确核验**(校验层):按 `law_name + article_no` 读源文件 md 取**精确原文** → fuzzy match 核验 LLM 引文。不依赖向量近似。

法条按"条"粒度 embedding 入库(每条款一条向量),非整部法一条。量级:第一版 ~300 条,Chroma 数千条无压力,用户灌库扩展可支撑上万条。
