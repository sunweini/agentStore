...
└──────────────────────────────┘
   ...
   │     失败退回 w3(needs_rework 恒映射 w3,重新生成)
   │
   ├─▶ [w5.5] 部署冒烟 worker(通用)★
   │     部署 DLL 到测试金蝶环境,验证 assembly 加载 + FormId→plugin 映射
   │     验证失败退回 w3(needs_rework 恒映射 w3,运行时验证,防"编译过跑不起来")
   │
   ├─▶ [w6] 打包 worker(通用)
...
| 库 | 内容 | 体量 | 检索方式 | 使用者 |
|---|---|---|---|---|
| API 参考库 | 类/方法签名、接口定义(Kingdee.BOS.*) | 大 | 向量 + BM25 混合(命名空间/FormId 过滤未实现,P2) | w2、w3、w4、w5 |
...
- **版本标注**:所有 API/指南条目带 K3Cloud 版本元数据,检索按目标版本过滤(轻量,不做完整兼容矩阵)。**实现状态:未实现(P2)** —— 当前 RAG 的 `filter` 仅支持简单 `{key: value}` 相等匹配(已接线:guide 按 plugin_type 过滤、经验库签名去重),版本/命名空间元数据标注与过滤尚未落地,见 tech.md §5。
...
| 金蝶 API 连不上 | 冒烟链路按 `KingdeeApiUnavailable` 处理 → BLOCKED;未配置/基础设施缺失 → 图包装器标记 failed(原设计"降级纯文本模式"未实现,P2) |
| 金蝶 API 限流(429) | 指数退避重试 2 次(1s/2s),仍败 → `KingdeeApiUnavailable`(无降级文本模式) |
| 审查退回超限(3 轮)/子任务退回上限 | **实现**:子任务 `max_rework` 超限 → 该子任务 failed 而非 needs_rework(环节级闸门);全局预算(≤3 轮)耗尽 → fail → w6_fail 收尾"未完成"包(原设计"主管汇总意见,问用户拍板"未实现) |
| **全局返工预算超限(总重新生成 ≤3 轮)** | 超限硬失败:fail → w6_fail 交付"未完成"包 `deliverable-failed-<ts>.zip`(部分产物 + compile_errors + 审查裁决含 Minor + 原因 + spec_version + 冻结 spec 快照),CLI/API 与正常交付包同一通道展示 |
| 编译超限(5 次) | 扣返工预算 → BLOCKED → needs_rework(退回 w3 重新生成);子任务上限耗尽 → failed;最终 fail 收尾"未完成"包含 compile_errors 日志 |
| 冒烟验证失败(assembly 未加载/映射错) | 扣返工预算 → BLOCKED → needs_rework(退回 w3),计入全局返工预算 |
| **编译服务不可用(容器挂/超时)** | w5 提交前先健康探测;服务故障直接报 BLOCKED,不算编译轮次不扣预算 → 图包装器标记 failed |
| **LLM 结构化输出失败(畸形 JSON)** | **实现**:解析失败(parsed=None 且无 tool_calls)重试 1 次(共 2 次尝试),仍败 → None → worker 确定性骨架降级(不报 BLOCKED,不中断图);重试与 load_skill 2 回合上限正交 |
...
| 时间预算超限 | 单轮编译 ≤2min,单任务编译 ≤15min,全流程 ≤30min 图级总闸:超限 → 剩余标记 failed → `fail:时间预算耗尽` → w6_fail 未完成包(原设计"强制升级问用户"未实现) |
| 任务进行中用户改需求 | 需求版本冻结(spec 确认即冻结,API 409),完成后开新任务重跑 |
...
- 指标:pass-rate(编译通过/任务总数)、返工轮次/任务,随 State 统计。**实现状态**:metrics 随 State 统计已落地(v1.10 `TaskState.metrics` 五计数器:compile/smoke 通过/失败 + rework_rounds,delta reducer);otel 仅打 span(主管决策 `kingdee.supervisor.decide` / worker 状态 `kingdee.worker.<name>` / 编译轮次 `kingdee.w5.compile_round`,低基数属性,遵循 OBS-CORE-003);指标数值的 otel 上报未实现(P3)。
...
- **部署拓扑**:docker-compose 编排三个服务 — api(FastAPI + 主管图)/ 编译容器(金蝶 DLL)/ RAG 存储(Chroma 目录挂载)。团队一键起,凭证 .env。**实现状态**:compose 目前只启用 compile-service(api 服务注释态、RAG 存储未挂载 —— Plan C 落地后未更新,见 tech.md §8.1);本地开发直接 `uvicorn "agents.kingdee_plugin_agent.api:create_app" --factory` 起 API。
- **可观测**:复用 common/otel.py — 主管派发/worker 状态变迁/编译轮次打 trace;**指标:pass-rate(编译通过/任务)、返工轮次/任务、冒烟通过率**随 State 统计(指标 otel 上报未实现,P3,见 §9 实现状态);TodoList 经 SSE 推 Web 端,失败子任务红标,审查退回可视化。
